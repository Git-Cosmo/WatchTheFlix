...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="text-sm text-gh-text-muted">
            {{ $media->total() }} {{ $media->total() === 1 ? 'show' : 'shows' }} found
        </div>
    </div>

    <!-- Enhanced Search and Filters -->
    <div class="mb-8 card p-6">
        <form method="GET" action="{{ route('tv-shows.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gh-text-muted mb-2">Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search TV shows..." class="input-field w-full">
                </div>
                <div>
            ...
            <div class="flex gap-3">
                ...
                @if(request()->hasAny(['search', 'genre', 'sort', 'platform', 'year_from', 'year_to', 'min_rating']))
                <a href="{{ route('tv-shows.index') }}" class="btn-secondary">
                    <svg class="h-4 w-4 mr-2 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
